...
These are my detailed notes for the harmonic oscillator problem from lecture. I first work through the one-dimensional translated ground state, and then I add the three-dimensional version because that is the natural next step before starting numerical methods. The goal is to connect the lecture-note state
